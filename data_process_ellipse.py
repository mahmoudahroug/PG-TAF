...
# major axis (front) = 15 meters and minor axis (sides) = 5 meters 
...
def process_data(pra_now_dict, pra_start_ind, pra_end_ind, pra_observed_last):
	...
	positions = xy  # (N, 2)
	headings = visible_object_value[:, 9]  # assuming heading is at index 9
	for i in range(num_visible_object):
		x0, y0 = positions[i]
		theta = headings[i]  # in radians
		...
		for j in range(num_visible_object):
			...
			x, y = positions[j]
			dx = x - (x0 + forward_offset * cos_theta)
			dy = y - (y0 + forward_offset * sin_theta)

			# Rotate into ego frame
			rel_x = dx * cos_theta + dy * sin_theta
			rel_y = -dx * sin_theta + dy * cos_theta

			# Elliptical neighbor check
			if rel_x < back_clip_at:
				neighbor_matrix[i, j] = 0
			elif (rel_x / ellipse_front_distance) ** 2 + (rel_y / ellipse_sides_distance) ** 2 < 1:
...
def generate_data(pra_file_path_list, pra_is_train=True):
	...
	if pra_is_train:
		save_path = 'train_data_ellipse.pkl'
	else:
		save_path = 'test_data_ellipse.pkl'
