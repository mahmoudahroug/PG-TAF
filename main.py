...
def preprocess_data(pra_data, pra_rescale_xy):
	...
	print(object_type.shape, object_type[0])
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
    ...
    for iteration, (ori_data, A, _) in enumerate(pra_data_loader):
        ...
        # for now_history_frames in range(6, 7):
            ...
            predicted = pra_model(pra_x=input_data, pra_A=A, pra_pred_length=output_loc_GT.shape[-2], agent_type_ids=object_type, pra_teacher_forcing_ratio=0, pra_teacher_location=output_loc_GT) # (N, C, T, V)=(N, 2, 6, 120)
            ########################################################
...
def test_model(pra_model, pra_data_loader):
	...
	with open(test_result_file, 'w') as writer:
		...
		for iteration, (ori_data, A, mean_xy) in enumerate(pra_data_loader):
			...
			A = A.float().to(dev)
			predicted = pra_model(pra_x=input_data, pra_A=A, pra_pred_length=future_frames, agent_type_ids=object_type, pra_teacher_forcing_ratio=0, pra_teacher_location=None) # (N, C, T, V)=(N, 2, 6, 120)
			predicted = predicted *rescale_xy 
...
if __name__ == '__main__':
    ...
    pretrained_model_path = './trained_models/model_epoch_0008.pt'
